    ...
    <section className="py-20">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <AnimateOnScroll animation="fade-up">
          <SectionHeading
            title={title}
            subtitle="From classic photo strips to 360 videos, we have the perfect booth for your event."
          />
        </AnimateOnScroll>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
          {booths.map((booth, i) => (
            <AnimateOnScroll key={booth.id} animation="fade-up" delay={i * 100}>
              {/* TiltCard wraps the entire card for the 3D perspective effect */}
              <TiltCard className="rounded-2xl overflow-hidden h-full">
                <Link
                  href={`/book?booth=${booth.id}`}
                  className="group block bg-white rounded-2xl overflow-hidden shadow-sm border border-gray-100 h-full"
                >
                  {/* Image placeholder */}
